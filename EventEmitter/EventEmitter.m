...
#pragma mark - on sender

- (void)on:(NSString*)event sender:(id)sender notify:(EventEmitterNotifyCallback)callback
{
	[sender addListener:[[EventEmitterNotifyCallbackListener alloc] init] callback:callback event:event once:NO];
}

- (void)on:(NSString*)event sender:(id)sender callback:(EventEmitterDefaultCallback)callback
{
	[sender addListener:[[EventEmitterDefaultCallbackListener alloc] init] callback:callback event:event once:NO];
}

- (void)on:(NSString*)event sender:(id)sender array:(EventEmitterArrayCallback)callback
{
	[sender addListener:[[EventEmitterArrayCallbackListener alloc] init] callback:callback event:event once:NO];
}

- (void)once:(NSString*)event sender:(id)sender notify:(EventEmitterNotifyCallback)callback
{
	[sender addListener:[[EventEmitterNotifyCallbackListener alloc] init] callback:callback event:event once:YES];
}

- (void)once:(NSString*)event sender:(id)sender callback:(EventEmitterDefaultCallback)callback
{
	[sender addListener:[[EventEmitterDefaultCallbackListener alloc] init] callback:callback event:event once:YES];
}

- (void)once:(NSString*)event sender:(id)sender array:(EventEmitterArrayCallback)callback
{
	[sender addListener:[[EventEmitterArrayCallbackListener alloc] init] callback:callback event:event once:YES];
}

#pragma mark - Internal helper function
...
- (NSArray*)listeners
{
    // get all listerners
    NSMutableDictionary* eventListeners = objc_getAssociatedObject(self, _EventEmitter_ListenerArray);
    NSArray *allValues = [eventListeners allValues];
    NSMutableArray *allListerners = [NSMutableArray array];
    for (NSArray *eventListeners in allValues)
    {
        [allListerners addObjectsFromArray:eventListeners];
    }

    // remove duplicated listeners
    NSCountedSet *countedSet = [NSCountedSet setWithArray:allListerners];
    NSMutableArray *nonRedundantListeners = [NSMutableArray arrayWithCapacity:[allListerners count]];
    
    for(id obj in countedSet)
    {
        if([countedSet countForObject:obj] == 1)
        {
            [nonRedundantListeners addObject:obj];
        }
    }
    
    return nonRedundantListeners;
}
